...
abbreviation=kpk
data_dir=/srv/data1/speech/public/kpk_data/
csv_dir=$data_dir/csv/
kaldi_kit=$data_dir/kaldi_kit/
text_train_dir=$kaldi_kit/text_train_dir/
text_test_dir=$kaldi_kit/text_test_dir/
...
if [ $stage -le 1 ]; then
	...
	for csv in $csv_dir/*; do
		if [ ${csv: -4} == ".csv" ]; then
			...
			curr_file=${temp/'.csv'/''}
			echo "$csv"
			python3 kak_fanda/parse_csv/main.py $abbreviation $csv > $log_dir/parse_csv_log.${curr_file}.txt
