...
def get_args():
    ...
    parser.add_argument("--min-output-duration", type=float, default = 26.0,
    ...
    parser.add_argument("file_id", type=str, metavar="<file-id>")
...
def create_vad_wav(x, sr, voiced, step):
    count = 0
    splice = 5
    limits = []
    while count < len(voiced) - 1:
        ...
        cur_x = []
        count_temp = 0
        while not voiced[count] and count < len(voiced) - 1:
    ...
    # Get final segments
    segments = []
    segment_temp = np.asarray([], dtype=np.int16)
    for l in limits:
        segment_temp = x[l[0]:l[1]]
...
def main():
    ...
    try:
        ...
        try:
            ...
            segments = create_vad_wav(x, sr, voiced, conf.frame_shift / 1000.0)
        except Exception as e:
            _, _, exc_tb = sys.exc_info()
            print('ERROR (main():{}:{:d}) {}'.format(script, exc_tb.tb_lineno, e), file=sys.stderr)
            print('WARNING (main():{}:{:d}) Failed to compute silent for utterance {}'.format(script,
                exc_tb.tb_lineno, args.wav), file=sys.stderr)
        else:
            if not os.path.exists(args.output):
                os.system('mkdir {}'.format(args.output)) 
            # else:
            #     os.system('rm {}/*'.format(args.output)) # remove all files in the output dir
                
            for i in range(len(segments)):
                if segments[i].shape[0] >= sr * args.min_output_duration:
                    output = args.output + '/silent_' + args.file_id + '_' + str(i) + '.wav'
                    wav.write(output, sr, segments[i])
                    # print('VLOG[2] (main():{}:354) Processed silent for {}'.format(script, args.wav),file=sys.stderr)
                # else:
                #     print('WARNING (main():{}:357) Minimum duration of {}s not reached for {}'.format(
                #         script, args.min_output_duration, args.wav), file=sys.stderr)
