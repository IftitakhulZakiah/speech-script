...
def create_vad_wav(x, sr, voiced, step, silence_min):
    ...
    # Get final segments
    audios_temp = [x[l[0]:l[1]] for l in limits]
    # audios = audios_temp

    mergings = merge_gap(limits,sr,silence_min)
    audios = []
    for merging in mergings:
        segments = np.asarray([], dtype=np.int16)
        for audio_idx in merging:
            segments = np.concatenate((segments, audios_temp[audio_idx]))
        audios.append(segments)
    
    audios = [audio for audio in audios if len(audio) > sr ]
...
def main():
    ...
    try:
        ...
        try:
            ...
            i = 1
            print('INFO write segmented file')
