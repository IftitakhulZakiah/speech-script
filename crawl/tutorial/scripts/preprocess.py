    ...
    for i, match in enumerate(matches):
        ...
        if context[i] != "" and context[i+1] != "":
            ...
            if context[i][-1] == 'H' and context[i+1][0] in string.digits:
                ...
                # print(sentence)
                left_cont = context[i].split(" ")[-1]
                # print("----")
                # print(sentence)
                # print(context)
                # print(context[i+1].split(" ")[0])
                right_cont = context[i+1].split(" ")[0]
...
if __name__ == '__main__':
    ...
    output = args.input.replace(".txt", "_prep.txt")
    nthreads = 20

    try:
        sentences = open(args.input).read().splitlines()
    ...
    for idx, line in enumerate(sentences):
        if "girlban d (G)I-DLE" in line:
            sentences[idx] = line.replace("girlban d (G)I-DLE", "girlband (G)I-DLE") 

    if nthreads != None:
        nchunk = int(len(sentences) / (nthreads * 2))
